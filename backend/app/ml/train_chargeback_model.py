"""SENTINEL ML - Chargeback Risk Classifier Training Script

Trains a Random Forest classifier on 30 transaction features to predict
the propensity of a transaction resulting in a chargeback or dispute.
...
def train_model():
    print("=" * 60)
    print("  SENTINEL - Chargeback Risk Classifier Training (Random Forest)")
    print("=" * 60)
    ...
    y = df_trainable['chargeback_label'].values
    ...
    model_path = os.path.join(model_dir, 'chargeback_classifier.joblib')
    ...
    meta_path = os.path.join(model_dir, 'chargeback_model_meta.json')
    with open(meta_path, 'w') as f:
        json.dump(meta, f, indent=2)
    print(f"  Metadata saved to: {meta_path}")

    print("\n" + "=" * 60)
    print("  [OK] Chargeback classifier training complete!")
    print("=" * 60)
    return model
